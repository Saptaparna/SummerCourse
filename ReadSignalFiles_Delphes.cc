...
int ReadSignalFiles_Delphes(std::string infile, std::string outfile, std::string channel){
  ...
  for (int i=0; i<nEvents ; ++i)
   ...
   int emu_event = 0;
   if(mu1_p4.Pt()>5.0){
      if(el1_p4.Pt()>10.0 and mumu_event==0) {
        emu_event = 1;
        TVector2 mu1_transverse;
   ...
   int elel_event = 0;
   if(el1_p4.Pt()>10.0){
      if(el2_p4.Pt()>10.0 and mumu_event==0 and emu_event==0) {
        elel_event = 1;
        TVector2 el1_transverse;
